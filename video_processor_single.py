        ...
        with tqdm(total=total_frames, desc="处理视频") as pbar:
            while cap.isOpened():
                ...
                if len(results) > 0:
                    ...
                    # 只取面积最大的一个
                    if all_detections:
                        largest_detection = all_detections[0]
                        
                        # 提取信息
                        x1, y1, x2, y2 = largest_detection['bbox']
                        conf = largest_detection['confidence']
                        cls_id = largest_detection['class_id']
                        kpts = largest_detection['keypoints_data']
                        
                        # 从detection中移除临时存储的原始关键点数据
                        del largest_detection['keypoints_data']
                        
                        # 添加到JSON数据
                        frame_data['detections'].append(largest_detection)
                        
                        # 绘制边界框 - 红色表示最大的目标
                        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
                        
                        # 绘制类别和置信度
                        cv2.putText(frame, f"{cls_id} {conf:.2f} Area:{largest_detection['area']:.0f}", (x1, y1 - 5), 
